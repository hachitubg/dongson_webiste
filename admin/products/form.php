...
// Get product categories - only parent categories (2-level structure)
$parentCategoriesStmt = $pdo->prepare('SELECT * FROM categories WHERE type = ? AND is_active = 1 AND parent_id IS NULL ORDER BY sort_order ASC, name ASC');
$parentCategoriesStmt->execute(['product']);
$parentCategories = $parentCategoriesStmt->fetchAll();

// Get all child categories grouped by parent
$childCategories = [];
$childCategoriesStmt = $pdo->prepare('SELECT * FROM categories WHERE type = ? AND is_active = 1 AND parent_id IS NOT NULL ORDER BY parent_id, sort_order ASC, name ASC');
$childCategoriesStmt->execute(['product']);
$allChildren = $childCategoriesStmt->fetchAll();
foreach ($allChildren as $child) {
    $parentId = $child['parent_id'];
    if (!isset($childCategories[$parentId])) {
        $childCategories[$parentId] = [];
    }
    $childCategories[$parentId][] = $child;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$category_id) {
        ...
        // Validate that selected category is a child category (has parent_id)
        $catCheck = $pdo->prepare('SELECT parent_id FROM categories WHERE id = ? LIMIT 1');
        $catCheck->execute([$category_id]);
        $selectedCat = $catCheck->fetch();
        if (!$selectedCat || $selectedCat['parent_id'] === null) {
            $errors[] = 'Vui lòng chọn danh mục con (không thể chọn danh mục cha)';
        }
    }
    ...
    if ($isEditMode) {
        ...
        $upd = $pdo->prepare('UPDATE products SET title = ?, slug = ?, category_id = ?, short_description = ?, description = ? WHERE id = ?');
        $upd->execute([$title, $slug, $category_id, $short_description, $description, $postId]);
        $productId = $postId;
    } else {
        ...
        $ins = $pdo->prepare('INSERT INTO products (title, slug, category_id, short_description, description) VALUES (?, ?, ?, ?, ?)');
        $ins->execute([$title, $slug, $category_id, $short_description, $description]);
...
<body>
      ...
      <div class="form-row">
        ...
        <div id="category-selection">
          <div class="form-row" style="margin-bottom:12px">
            <label style="font-size:13px;color:#666">Bước 1: Chọn nhóm sản phẩm</label>
            <select id="parent_category" onchange="loadChildCategories()" style="width:100%">
          ...
          <div class="form-row" id="child-category-row" style="display:none">
            <label style="font-size:13px;color:#666">Bước 2: Chọn danh mục cụ thể <span class="required">*</span></label>
            <select name="category_id" id="category_id" required style="width:100%">
              <option value="">-- Chọn danh mục --</option>
            </select>
          </div>
        </div>
        <span class="form-error">Vui lòng chọn danh mục sản phẩm</span>
      </div>

      <div class="form-row">
        <label>Mô Tả Ngắn</label>
        <textarea name="short_description" rows="3" placeholder="Mô tả ngắn gọn về sản phẩm (hiển thị trong danh sách)"><?php echo htmlspecialchars($formData['short_description'] ?? $product['short_description'] ?? ''); ?></textarea>
        <p class="muted">Mô tả ngắn gọn, tóm tắt về sản phẩm</p>
      </div>
  ...
  <script>
    // Child categories data from PHP
    const childCategoriesData = <?php echo json_encode($childCategories); ?>;
    const currentCategoryId = <?php echo json_encode($formData['category_id'] ?? $product['category_id'] ?? null); ?>;
    
    CKEDITOR.replace('description');

    // Load child categories based on parent selection
    function loadChildCategories() {
      const parentSelect = document.getElementById('parent_category');
      const childRow = document.getElementById('child-category-row');
      ...
      if (children.length === 0) {
        childRow.style.display = 'none';
        alert('Nhóm sản phẩm này chưa có danh mục con. Vui lòng thêm danh mục con trước.');
        parentSelect.value = '';
        return;
      }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      if (currentCategoryId) {
        ...
        for (const parentId in childCategoriesData) {
          const children = childCategoriesData[parentId];
          const found = children.find(c => c.id == currentCategoryId);
          if (found) {
            document.getElementById('parent_category').value = parentId;
            loadChildCategories();
            setTimeout(function() {
              document.getElementById('category_id').value = currentCategoryId;
            }, 100);
            break;
          }
        }
